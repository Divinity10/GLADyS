-- GLADyS Memory Subsystem - Initial Schema (NO PGVECTOR)
-- This is a simplified schema for local development without pgvector.
-- For production, use 001_initial_schema.sql which requires pgvector.

-- Enable required extensions (uuid-ossp only, no vector)
...
CREATE TABLE episodic_events (
    ...
    -- Embedding stored as array (no vector ops without pgvector)
...
-- Indexes for episodic_events (no HNSW index without pgvector)
...
-- Indexes for entities (no HNSW index without pgvector)
...
-- Indexes for user_profile (no HNSW index without pgvector)
