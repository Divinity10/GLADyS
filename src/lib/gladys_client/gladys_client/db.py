...
and heuristic_fires. Used by the dashboard backend, CLI, and sensors.

Connection uses psycopg2 (synchronous) with DSN built from explicit
port parameter — no coupling to CLI environment config.
"""
...
def get_dsn(port: int) -> str:
    """Build a psycopg2 DSN string for the given port."""
